...
const GoBackButton : FC<GoBackButtonProps> = ({className, modifier = 0, to, children, ...rest}) => {
    const navigate = useNavigate()

    return (
        <button onClick={() => {return to ? navigate(to) : navigate((-1 - modifier))}} className={`${s.back__btn} ${className && className}`} {...rest}>
            <svg
